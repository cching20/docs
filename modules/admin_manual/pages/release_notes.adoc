...
- To prepare for the new background job for change detection in federated shares (xref:release_notes.html#new-background-job-for-change-detection-in-federated-shares[see below]), a https://github.com/owncloud/core/pull/37391[migration step] adds a new column (`lastscan`) to the `oc_share_external` table. The impact on upgrade duration depends on the number of rows in `oc_share_external`.
...
- To facilitate the transition to the new licensing mechanism (xref:release_notes.html#changes-in-delivery[see below]) a https://github.com/owncloud/core/pull/37512[migration step] will disable the `enterprise_key` app during the upgrade. This step does not have an impact on upgrade duration.

=== PHP 7.1 Support Discontinued

As xref:release_notes.html#php-71-deprecation-note[announced], in the previous minor release of ownCloud Server, from version 10.5 onward, ownCloud Server **no longer supports PHP 7.1**. 
...
TIP: See the xref:installation/system_requirements.adoc#officially-recommended-supported-options[system requirements in the ownCloud Documentation] for more information.
...
To prevent files being locked infinitely, there is a mechanism that automatically expires locks after a certain time. The expiration time of locks can be configured via the "Manual File Locking" section in the _Admin -> Additional_ settings or using _occ_ commands:
...
TIP: The user-facing components in the web interface are disabled by default. Administrators can enable the feature by executing the following _occ_ command: `occ config:app:set files enable_lock_file_action --value true`

=== Changes to the ownCloud Marketplace

The ownCloud Marketplace is opening up. With the release of Server 10.5, all apps available on the Marketplace (including ownCloud Enterprise apps) are available for download and installation via the Market app, respectively. This change facilitates the process of getting started with ownCloud Server and of evaluating Enterprise functionality a lot. Additionally, it allows to obtain updates for all apps from the ownCloud Marketplace and ensures running up-to-date versions of the extensions thereby.
...
The availability of Enterprise features eventually only depends on the license key.
As part of this process, the former `enterprise_key` app has been deprecated and is not shipped anymore. New  versions for all Enterprise apps have been released and included in the Server 10.5 complete bundle which are needed to work with PHP 7.4 and the new licensing mechanism (see below).
...
TIP: It is mandatory to disable the deprecated `enterprise_key` app when upgrading. A migration step will conduct this automatically during the upgrade procedure.

TIP: Practically, for existing installations the change does not make a big difference. Community installations will get all supported Community and Enterprise apps in addition but they will be disabled. For Enterprise installations the bundles stay equal. After upgrading it is recommended to check whether the desired apps are enabled/disabled.
...
With Server 10.5 this process has been changed and made a lot easier: Along with the changes to the Marketplace and in delivery, Enterprise functionality is basically available in every installation after upgrading or installing, respectively. The apps can be enabled like other apps which will start a 24-hour grace period if no valid license key is present. During this time the functionality can fully be used, even without a valid license key. To extend the period to 30 days, a demo license key can be obtained from a https://owncloud.com/try-enterprise/[new landing page]. The administrator will be informed and guided to obtain the demo license and can enter it in the web interface. If desired, the location of the landing page can be customized using the _config.php_ option `'grace_period.demo_key.link' => 'https://owncloud.com/try-enterprise/'`.

=== New Admin UI to Supply License Keys

As mentioned above, Server 10.5 adds new UI elements to set license keys in the _Admin -> General_ settings. Keys added this way will be stored in the database, not in _config.php_ as before. Still, there is legacy support taking into account when keys have been stored in _config.php_ or _license.config.php_.

=== New Background Job for Change Detection in Federated Shares

With ownCloud Server 10.2.1 a https://doc.owncloud.com/server/admin_manual/release_notes.html#background-job-for-change-detection-of-nested-federated-shares[background job for change detection of nested federated shares] has been added (`occ incoming-shares:poll`) to allow ownCloud Server to discover changes in federated shares in order to make them available for synchronization with the ownCloud Clients. Based on feedback a new, improved background job with more configuration options was added to Server 10.5. It replaces the former occ command which is now **deprecated** and should not be used anymore after upgrading to 10.5.
...
- Minimum amount of hours since the last login of a user for that a scan is triggered (limits the execution of discovery & meta data sync to active users which have logged in during the configured time frame) (default: 24h)
...
The new background job is disabled by default and can be enabled/disabled in the _Admin -> Sharing_ settings (_Periodically synchronize outdated federated shares for active users_) or using `occ config:app:set files_sharing cronjob_scan_external_enabled --value yes`. If enabled, it will be executed as part of the regular ownCloud background job queue and does not need to be added to crontab therefore.
...
- The "Add to your ownCloud" feature on public link pages is now hidden if an instance has outgoing federated shares disabled (specifically, if _Allow users on this server to send shares to other servers_ is disabled in Admin -> Sharing settings) https://github.com/owncloud/core/pull/37232[#37232]
...
- The xref:release_notes.html#sharing-with-numeric-uids[known issue] around sharing with numeric UIDs in 10.4.0 and 10.4.1 has been fixed. https://github.com/owncloud/core/pull/37336[#37336]
